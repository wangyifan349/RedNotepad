...
class RedNotepad(QMainWindow):
    ...
    SENSITIVE_MAGIC = b"REDNOTEPAD-AESGCM-256-V1\n"
    ...
    def create_actions(self):
        ...
        self.open_sensitive_action = QAction("Open Sensitive Data", self)
        ...
        self.sensitive_mode_action = QAction("Sensitive Data Mode", self)
    ...
    def open_file(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(
            ...
            "Open File",
            "",
            "Supported Files (*.txt *.log *.json *.xml *.csv *.py *.md *.ini *.cfg *.rns);;"
            "Sensitive Data (*.rns);;Text Files (*.txt *.log *.json *.xml *.csv *.py *.md *.ini *.cfg);;"
            "All Files (*.*)",
        )
    ...
    def sensitive_password_message(self, action):
        return (
            "Sensitive Data Mode uses AES-256-GCM encryption.\n\n"
            "This feature is not recommended for ordinary notepad use. This encrypted format is not guaranteed to be compatible with other "
            "editors or applications. After upload to cloud storage, the cloud service normally cannot directly preview, index, or "
            "parse the plaintext content. Unless you genuinely need to store sensitive information, continue using the normal "
            "text format.\n\n"
            "Keep the password safe: it is not written into the file, and forgotten passwords cannot be recovered.\n\n"
            f"{action} password:"
        )
    ...
    def prompt_sensitive_password(self, action):
        ...
        if not password:
            QMessageBox.warning(self, "Password Required", "The sensitive-data password cannot be empty.")
            return None

        return password

    def prompt_new_sensitive_password(self):
        password = self.prompt_sensitive_password("Enter")
        if password is None:
            return None

        confirmation, accepted = QInputDialog.getText(
            self,
            "Confirm Sensitive Data Password",
            "Enter the same password again.\n\n"
            "The password is not stored in the application configuration. If you forget it, the encrypted content cannot be recovered.",
            QLineEdit.EchoMode.Password,
        )
        ...
        if password != confirmation:
            QMessageBox.warning(self, "Passwords Do Not Match", "The two passwords do not match.")
            return None
    ...
    def decrypt_sensitive_data(self, file_data, password):
        ...
        if len(file_data) < minimum_size:
            raise ValueError("The sensitive-data file format is incomplete.")

        if not file_data.startswith(self.SENSITIVE_MAGIC):
            raise ValueError("This is not a RedNotepad sensitive-data file.")
        ...
        if iterations < 100000 or iterations > 2000000:
            raise ValueError("The sensitive-data file contains invalid key-derivation parameters.")
    ...
    def open_sensitive_file(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(
            ...
            "Open Sensitive Data",
            "",
            "RedNotepad Sensitive Data (*.rns);;All Files (*.*)",
        )
    ...
    def open_sensitive_file_path(self, path):
        ...
        try:
            ...
            QMessageBox.critical(
                self,
                "Decryption Failed",
                "The password is incorrect, the file is damaged, or the file is not a supported sensitive-data format.",
            )
            return
        ...
        self.encoding_label.setText("Mode: AES-256-GCM")
    ...
    def enter_sensitive_mode(self):
        if self.loading_file is not None:
            QMessageBox.information(
                self,
                "File Is Still Loading",
                "Please wait until the large file has finished loading before entering Sensitive Data Mode.",
            )
            return
    ...
    def update_title(self):
        ...
        sensitive = " [Sensitive Data]" if self.sensitive_mode else ""
